...
%
% Assumptions:
% 1) control screens queries: "undefined_sn4757"
...
if(~isempty(strfind(matlab_version, version_blacklist{1}))) % loop this if blacklist g`
...
sgadata.spatialnorm_colsize = ...
    apply_spatial_normalization(sgadata, 'colsize_platenorm', ignore_cols, plate_id_map);
...
for i = 1:length(all_querys)
    ...
    for k = 1:length(curr_plates)
        ...
        for j = 1:length(curr_arrays)   
            ...
            t = find(tot_dev - jackknife_dev > 0.9*tot_dev); 
...
for i = 1:length(all_querys)
    ...
    for j = 1:length(all_arrayplateids)
        ...
        for k = 1:length(currsets)
            ...
            %measure of variance, used to estimate some pvalue of how
            %reproducable it is
...
for i = 1:length(all_arrays)
    ...
    sgadata.arraymean_corrected(all_ind) = nanmean(query_effects);
