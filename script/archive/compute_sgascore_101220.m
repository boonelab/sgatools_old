...
for i = 1:length(all_querys)
    ...
    for k = 1:length(curr_plates)
        ...
        curr_mat1 = zeros(length(curr_arrays),max_cols)+NaN;
        curr_mat2 = zeros(length(curr_arrays), max_cols)+NaN;
        ...
        for j = 1:length(curr_arrays)   
            ...
            curr_mat1(j,1:length(ind2)) = sgadata.(field)(ind(ind2));
            curr_mat2(j,1:length(ind2)) = sgadata.(field)(ind(ind2));
            ...
            if length(t) <= 0.25 * length(vals)            
                result_new2(ind_mat(j,ind3(t))) = NaN;
                curr_mat2(j,ind3(t)) = NaN;
                end
