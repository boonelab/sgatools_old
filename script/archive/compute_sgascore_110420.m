...
vars_to_check = {'inputfile','linkagefile','smfitnessfile'};
...
% Get colonies corresponding to questionable arrays (TEMP)
dat = importdata('remove_array_list_definite.txt');
[t, ind1, ind2] = intersect(sgadata.orfnames, dat);
ignore_cols1 = find(ismember(sgadata.arrays, ind1));

dat = importdata('remove_array_list_maybe.txt');
[t, ind1, ind2] = intersect(sgadata.orfnames, dat);
ignore_cols2 = find(ismember(sgadata.arrays, ind1));

dat = importdata('remove_query_list_definite.txt');
[t, ind1, ind2] = intersect(sgadata.orfnames, dat);
ignore_cols3 = find(ismember(sgadata.querys, ind1));

dat = importdata('remove_query_list_maybe.txt');
[t, ind1, ind2] = intersect(sgadata.orfnames, dat);
ignore_cols4 = find(ismember(sgadata.querys, ind1));
...
ignore_cols = unique([ignore_cols1; ignore_cols2; ignore_cols3; ignore_cols4; all_linkage_cols]);
...
fprintf(['Getting the number of good colonies per array...\n|' blanks(50) '|\n|']);
for i = 1:length(all_arrays)
